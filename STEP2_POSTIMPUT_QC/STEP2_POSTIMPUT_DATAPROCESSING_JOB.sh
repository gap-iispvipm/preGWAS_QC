...
# ===================================
# SELECT BATCHES - NON INTERACTIVE JOB MODE
# ===================================
...
if [ "$BATCH_MODE" == "ALL" ]; then
...
elif [ "$BATCH_MODE" == "SELECTED" ]; then

    if [ -z "$BATCH_LIST" ]; then
        echo "ERROR: BATCH_MODE=SELECTED but BATCH_LIST is empty"
        exit 1
    fi
    ...
    echo "ERROR: invalid BATCH_MODE: $BATCH_MODE"
    echo "Use BATCH_MODE=ALL or BATCH_MODE=SELECTED"
    exit 1
fi
...
for chr in {1..22}; do
    ...
    unzip -P "$PASSWORD" -o "$ZIPFILE" -d "$D"
